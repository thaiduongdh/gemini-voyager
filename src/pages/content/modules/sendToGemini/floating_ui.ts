...
function initFloatingUI() {
    ...
    function loadQueue(): void {
        chrome.storage.local.get(['stg_videoQueue'], (result) => {
            updateQueueUI(result.stg_videoQueue || []);
        });
    }
    ...
    chrome.storage.onChanged.addListener((changes, area) => {
        if (area === 'local' && changes.stg_videoQueue) {
            updateQueueUI(changes.stg_videoQueue.newValue || []);
        }
        if (area === 'local' && changes.stg_showAdvancedMenu) {
            updateAdvancedToggleButton(changes.stg_showAdvancedMenu.newValue);
        }
    });
    ...
    btnClear.addEventListener('click', () => {
        chrome.storage.local.get(['stg_videoQueue'], (result) => {
            if (result.stg_videoQueue?.length > 0) {
